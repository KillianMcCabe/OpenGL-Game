...
void Fountain::init() {
	...
	//programID = LoadShaders( "Particle.vertexshader", "Particle.fragmentshader" );
	{
		...
		assert (parse_file_into_str2 ("Particle.vertexshader", vertex_shader_str, 81920));
		assert (parse_file_into_str2 ("Particle.fragmentshader", fragment_shader_str, 81920));
		GLuint vs, fs;
	...
	GLubyte *image_data;
	// body
	image_data = stbi_load("assets/particle.png", &tex_width, &tex_height, &components, 4);
	printf("%s texture width, height, components: %d, %d, %d\n", "particle texture", tex_width, tex_height, components);
...
void Fountain::draw(float delta, mat4 V, mat4 P, vec3 light_pos) {
		...
		glm::mat4 ViewProjectionMatrix = P * V;
		...
		for(int i=0; i<newparticles; i++){
			...
			ParticlesContainer[particleIndex].pos = glm::vec3(-5,0,-5.0f);
